 ...
 * @property {string} taskContext.attachmentStore - `Map` instance used to store artifacts (e.g. blobs, files) generated by a task
 *
 ...
 *
 * @property {(options) => void} addAttachmentFn adds an attachment to the running task
 * 
 ...
 */
class TaskHandle {
  constructor(progressFn, addAttachmentFn, taskContext) {
     ...
     */
    this.addAttachment = ({ data, mimeType, name }) => {
      addAttachmentFn({
        name,
        data,
        mimeType,
        key: `${taskContext.id}:${name}`,
...
export class Task {
  ...
  constructor(taskFn, taskName, workflowConfig={}) {
    ...
    this.#taskHandle = new TaskHandle(
      this.#onTaskProgress.bind(this), 
      this.#onAddAttachment.bind(this), 
    this);
  ...
  get attachments() {
    return Array.from(this.#attachmentStore.values()).map((val) => {
      const { data, ...rest } = val;
      return rest; 
    });
  }
   ...
   */
  #onAddAttachment({ key, data, mimeType, name }) {
    this.#attachmentStore.set(key, {
      data,
      mimeType,
      name,
      taskId: this.#id,
