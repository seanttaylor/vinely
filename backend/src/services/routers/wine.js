...
export class WineRouter {
  ...
  constructor({ Events, MiddlewareProvider, WineService }) {
          ...
          .map((exceptionEvent) =>
            Result.error(
              Problem.of({
                ...
                detail:
                  "There was an error while fetching the requested resource (wines).",
     ...
     * @param {Function} next
     */
    router.post("/wines", MiddlewareProvider.Validation.validateRequestBody, async (req, res, next) => {
      try {
        const queryResult = await WineService.create(req.body);
        const { success: onQuerySuccess, error: onQueryError } =
          HTTPResponse.with(res);
